  ...
  methods: {
    getSurface(url) {
      ...
      if (sp.isNullOrEmpty(downloadUrl)) {
        return import('../../../../assets/images/pic_err.png');
      }
      return downloadUrl;
    },
    onSearch(value) {
    ...
    fetchData() {
      try {
        sp.get(
          `api/post/search?pageSize=${this.pageSize}&pageIndex=${this.pageIndex}&searchList=&viewId=${this.viewId}&searchValue=${this.searchValue}`
        ).then(resp => {
